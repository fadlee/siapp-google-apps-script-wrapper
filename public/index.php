...
require_once __DIR__ . '/../config/AppManager.php';

// Load data from SleekDB database
function loadAppData() {
    try {
        $appManager = new AppManager();
        return $appManager->getAllApps();
    } catch (Exception $e) {
        error_log('Error loading app data: ' . $e->getMessage());
        return [];
    }
}
...
// Find app data by slug using AppManager
function findAppBySlug($slug, $data = null) {
    if (empty($slug)) return null;

    try {
        $appManager = new AppManager();
        return $appManager->getAppBySlug($slug);
    } catch (Exception $e) {
        error_log('Error finding app by slug: ' . $e->getMessage());
        return null;
    }
}

// Get all available slugs using AppManager
function getAllSlugs($data = null) {
    try {
        $appManager = new AppManager();
        return $appManager->getAllSlugs();
    } catch (Exception $e) {
        error_log('Error getting all slugs: ' . $e->getMessage());
        return [];
    }
}
...
if (empty($route) || $route === 'index.php') {
    ...
    $landingPage = __DIR__ . '/../views/landing.html';
...
} elseif ($route === 'migrate.php' || $route === 'migrate') {
